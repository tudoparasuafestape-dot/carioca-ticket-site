...
/*
 * P0 PERFORMANCE:
 * analytics público permanece explicitamente desligado da jornada crítica.
 * Telemetria só poderá voltar em infraestrutura isolada do checkout/evento.
 */
if (!analyticsAsset || !/return;/.test(analyticsAsset)) {
  fail('assets/ct-analytics.js','analytics público deve permanecer desativado durante o P0 de performance');
}
for (const file of ['index.html','eventos-v2/index.html','evento-v2/index.html','checkout-v2/index.html','evento/index.html','checkout/index.html']) {
  ...
  if (html && html.includes('/assets/ct-analytics.js')) {
    fail(file,'página pública não pode carregar analytics durante o P0 de performance');
  }
}
