...
function contaCariocaPayFixture() {
  return {
    ...
    vendas: {
      confirmado: 90,
      vendasElegiveisAntecipacao: 600
    },
    ledger: {
      saldoOperacional: 600,
      porOrigem: {
        VENDAS_INGRESSOS_ONLINE: 90,
        VENDAS_PORTARIA: 0,
        VENDAS_BAR: 0,
        PATROCINIO: 0,
        CAPITAL_PROPRIO: 0,
    ...
    antecipacao: {
      ...
      baseElegivel: 600,
      maximoAntecipavel: 480,
      disponivelSolicitar: 480,
...
test.describe('Jornada operacional autenticada', () => {
  ...
  test('Portal -> Central -> Vendas -> Bar -> voltar -> logout sem transacao', async ({ page }) => {
    ...
    await expect(page.locator('#dashboard')).toBeVisible({ timeout: 15000 });
    await expect(page.locator('#confirmedSales')).toHaveText('R$ 90,00');
    await expect(page.locator('#ledgerBalance')).toHaveText('R$ 600,00');
    await expect(page.locator('#anticipationAvailable')).toHaveText('R$ 480,00');
    ...
    await expect(page.locator('#realMovement')).toHaveText('Desabilitada');
    await expect(page.locator('#providerAdvance')).toHaveText('Desabilitada');
    await expectNoTechnicalVisibleLinks(page);
